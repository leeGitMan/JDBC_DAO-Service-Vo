...
// * 여러 DML을 수행하지 않는 경우 (단일 DML , 단일 SELECT 등)라도
// 코드의 통일성을 지키기 위해서 Service에 작성
// Connection  객체는 여기서만 생성
...
public class TestService {
	...
	public int insert(TestVo vo1) throws SQLException{
		...
		// Connection conn = JDBCTemplate.getConnection(); --> 이거 좀 김, 
		// 그래서 위에 임포트 건드려서 밑에 처럼 사용가능
		
		Connection conn = getConncetion();
						// 클래스명.메서드명(JDBCTemplate)에서 만든 것.
		
		// INSERT DAO 메서드를 호출 하여, 수행 후 결과 반환 받기
	...
	public int insert(TestVo vo1, TestVo vo2, TestVo vo3) throws Exception{
		...
		}catch(SQLException e) { // dao 수행 중 발생 했을때, 
			rollback(conn);
